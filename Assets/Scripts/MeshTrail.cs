...
public class MeshTrail : MonoBehaviour
{
    ...
    public float speedBoost = 6;                                                        //잔상 효과 사용시에 속도 증가량
    ...
    private bool isTrailActive;                                                         //현재 잔상 효과가 활성화 되어 있는지를 확인 하는 변수
    ...
    IEnumerator ActivateTrail(float timeActivated)
    {
        ...
        while (timeActivated > 0)
        {
            if (skinnedRenderer == null)
                skinnedRenderer = positionToSpawn.GetComponentsInChildren<SkinnedMeshRenderer>();       //생성된 위치의 랜더러 컴토넌트들을 가져옴
